...
def main() -> None:
    ...
    per_event = hits.groupby("event_id").agg(
        ...
        detector_id =("detector_index", lambda x: x.unique().tolist()),
    )
    ...
    muonDat = pd.read_parquet("muon_truth.parquet")
    muonNeededDat = muonDat[["event_id","pos_x","pos_y","pos_z"]]
    totalPhotDat = by_detector #Initialize dataframe with needed data
    ...
    with open("PerDetectorHits.txt","w",encoding="utf-8") as file:
        print(by_detector,file=file)
